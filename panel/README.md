...
זרימת המשתמש בפועל כרגע: מודעה → `landing.html` → `thank-you.html` (וידאו של דוד) → `malchodet-hargaa.html` (המגנט: מלכודת ההרגעה) → **`masa-8-zehuyot.html`** (דף תוכנית הדגל, פאנל 1) → שיחת התאמה בוואטסאפ.
...
- ~~קישור מסדרת המיילים לדף התוכנית~~ — בוצע. כל ה-CTA-ים ב-`campaign-copy.md` (מיילים 1, 6, 7) מפנים עכשיו ל-`https://davidingber.github.io/panel/masa-8-zehuyot.html` במקום לוואטסאפ.
