...
| `thank-you.html` | דף תודה קצר (בסגנון `../anxiety-wave/thank-you.html`) — מברך ומפנה להדרכת הווידאו |
| `video-guide.html` | דף ההדרכה בווידאו — כאן יושב הסרטון של דוד, ובסופו מפנה להתחלת הבדיקה |
...
| `masa-8-zehuyot.html` | **פאנל 1** — דף תוכנית הדגל הקבוצתית, בת 8 שבועות. כאב, מנגנון, סילבוס כ-8 מעברי זהות, מחיר גלוי, למי מתאים/לא, שאלות נפוצות. ה-CTA מוביל כרגע לוואטסאפ ("שיחת התאמה") — עדיין אין שאלון התאמה דיגיטלי נפרד, ראו "מה עוד חסר" למטה. ⚠️ שם הקובץ נשאר `masa-8-zehuyot.html` (לא לשנות בלי בקשה של דוד), אבל התוכן על הדף (כותרת, H1, כפתור הוואטסאפ) עודכן ל-**"חוזרים לסמוך על עצמנו"** (21.8) — זה השם התקף בכל מקום שהדף מופיע, כולל הקישור מ-`malchodet-hargaa.html` וגם מ-`../anxiety-wave/` (ראו הערה למטה) |
...
זרימת המשתמש בפועל כרגע: מודעה → `landing.html` → `thank-you.html` (ברכה קצרה, מפנה להדרכה) → `video-guide.html` (וידאו של דוד) → `malchodet-hargaa.html` (המגנט: מלכודת ההרגעה) → **`masa-8-zehuyot.html`** (דף תוכנית הדגל, פאנל 1) → שיחת התאמה בוואטסאפ.
...
## חיבור הסרטון בדף ההדרכה

`video-guide.html` בנוי עם מקום לסרטון ההדרכה **"מה לעשות כשהחרדה עולה"** (שם שמקורו במסמך האסטרטגיה, לתוכן ההדרכה בפאנל 2 — לא להתבלבל עם המגנט "מלכודת ההרגעה" עצמו, ולא עם `hadracha/` שהיא כניסת לידים נפרדת לגמרי תחת אותו שם). `thank-you.html` עצמו הוא רק דף ברכה קצר שמפנה לכאן — הוא לא מכיל סרטון. עד שתחבר סרטון אמיתי, יוצג שם פלייסהולדר (לא תיבה שבורה).
...
3. הדבק אותו במקום `PASTE_YOUR_VIDEO_EMBED_URL_HERE` במשתנה `VIDEO_EMBED_URL` בתחילת ה-`<script>` התחתון בקובץ `panel/video-guide.html`.
...
- **הסרטון בדף ההדרכה** (`video-guide.html`) — ראה הוראות למעלה. עד שיחובר, מוצג פלייסהולדר.
